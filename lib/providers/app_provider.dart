...
class AppProvider extends ChangeNotifier {
  ...
  // Up to 3 user-configured SOS numbers (loaded from SharedPreferences)
  ...
  /// All 3 SOS number slots (may be empty strings for unused slots).
  List<String> get sosNumbers => List.unmodifiable(_sosNumbers);

  /// The first non-empty SOS number, or null if none is set.
  String? get primarySosNumber {
    for (final n in _sosNumbers) {
      if (n.trim().isNotEmpty) return n.trim();
    }
    return null;
  }

  /// Returns all non-empty SOS numbers.
  List<String> get activeSosNumbers => _sosNumbers
  ...
  static const _kSosPrefix = 'sos_number_';

  /// Persists [numbers] (up to 3) to SharedPreferences and notifies listeners.
  Future<void> setSosNumbers(List<String> numbers) async {
    final list = List<String>.generate(
      3,
      (i) => i < numbers.length ? numbers[i] : '',
    );
    _sosNumbers = list;
    final prefs = await SharedPreferences.getInstance();
    for (int i = 0; i < 3; i++) {
      await prefs.setString('$_kSosPrefix$i', list[i]);
    }
    notifyListeners();
  }

  /// Loads saved SOS numbers from SharedPreferences.
  Future<void> loadSosNumbers() async {
    final prefs = await SharedPreferences.getInstance();
    _sosNumbers = List<String>.generate(
      3,
      (i) => prefs.getString('$_kSosPrefix$i') ?? '',
    );
    notifyListeners();
  }
